...
/* Actual data sdio_write max retry. Each retry releases the bus lock and
 * sleeps 10 ms, so this bounds how long a single packet rides out a busy
 * (CPU-starved) C6 before being dropped. The C6 is single-core: a WiFi burst
 * can hold off the SDIO slave task for tens of ms, so allow ~100 ms of retry
 * to convert most would-be drops into slightly-delayed deliveries. */
#define MAX_SDIO_WRITE_RETRY              10
		...
		do {
			...
			if (ret) {
				...
				if (retries < MAX_SDIO_WRITE_RETRY) {
					ESP_LOGW(TAG, "%s: %d: retry after %d %ld", __func__,
						retries, ret, len_to_send);
					/* Release the lock before sleeping so the RX task can
					 * service the C6 and drain its SDIO buffers. */
					SDIO_DRV_UNLOCK();
					g_h.funcs->_h_msleep(10);
					SDIO_DRV_LOCK();
					/* Reset write position for a clean retry. */
					pos = sendbuf;
					data_left = len + sizeof(struct esp_payload_header);
					continue;
				} else {
					/* C6 temporarily busy (e.g. 802.11 association burst).
					 * SDIO bus is not stuck — drop this packet and let TCP
					 * retransmit rather than restarting the whole host. */
					SDIO_DRV_UNLOCK();
					ESP_LOGW(TAG, "SDIO write failed after %d retries; dropping packet",
						MAX_SDIO_WRITE_RETRY);
					goto done;
				}
			}
